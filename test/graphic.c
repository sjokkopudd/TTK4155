...
void convert_to_format (const uint32_t matrix[1][64], unsigned char output_matrix[8][1]) {

	FILE *output_file = fopen("firework4.txt", "w"); //this is to write to file output.
	...
	for (int column = 0; column < 8; column ++) {//fill list with 0b00000000
	...
	for (int row = 0; row < 1; row ++) {// fill list with data
	...
	for (int i = 0; i < 8; i++){// copy to output matrix
		for (int j = 0; j < 1; j++){
